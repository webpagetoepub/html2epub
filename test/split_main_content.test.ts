...
test("chapter elements contain content after the heading", () => {
  ...
  assert.ok(
    result[0].element
      .querySelector("p")!
      .textContent!.includes("Paragraph text"),
  );
  assert.ok(
    result[1].element.querySelector("p")!.textContent!.includes("Body text"),
  );
});
...
test("includes remaining content before first heading when it is at least 80 characters", () => {
  ...
  const main = makeElement(
    `<p>${longText}</p>` + "<h2>Chapter</h2><p>Chapter content.</p>",
  );
  ...
  assert.ok(
    ...
    "should include a chapter with metadata title",
  );
});
...
test("does not split at a heading that is empty", () => {
  ...
  assert.ok(
    ...
    "blank heading should not create a chapter",
  );
});
...
test("does not split at a heading that has only whitespace, tabs and line breaks", () => {
  ...
  assert.ok(
    !titles.some((title) => title.trim() === ""),
    "whitespace-only heading should not create a chapter",
  );
});
